...
namespace LuanPlatform.Core.Audio
{
    /// <summary>
    /// NAudio音频播放器
    /// </summary>
    internal sealed class NAudioPlayer
    {
        ...
        public int InvokeChannel()
        {
            ...
            {
                LogUtils.Log("Invoke Channel Failed." + ex.ToString(), "NAudioPlayer", LogLevel.Error);
                return 0;
            }
        }
        ...
        public bool InitAndPlay(int handle, MemoryStream ms, float vol, bool loop, bool isVocal = false)
        {
            try
            {
                ...
                {
                    LogUtils.Log("Play audio in empty channel:" + handle, "NAudioPlayer", LogLevel.Error);
                    return false;
                }
            ...
            {
                LogUtils.Log("Play audio failed." + ex, "NAudioPlayer", LogLevel.Warning);
                return false;
            }
        }
        ...
        public bool Fading(int handle, float destVol, int ms)
        {
            try
            {
                if (this.channelDict.ContainsKey(handle) == false)
                {
                    LogUtils.Log("Fade audio in empty channel:" + handle, "NAudioPlayer", LogLevel.Warning);
                    return false;
                }
            ...
            {
                LogUtils.Log("Fade audio failed." + ex, "NAudioPlayer", LogLevel.Warning);
                return false;
            }
        }
        ...
        public bool StopAndRelease(int handle)
        {
            try
            {
                if (this.channelDict.ContainsKey(handle) == false)
                {
                    LogUtils.Log("Stop audio in empty channel:" + handle, "NAudioPlayer", LogLevel.Error);
                    return false;
                }
            ...
            {
                LogUtils.Log("Stop and dispose audio failed." + ex.ToString(), "NAudioPlayer", LogLevel.Error);
                return false;
            }
        }
        ...
        public bool Pause(int handle)
        {
            try
            {
                if (this.channelDict.ContainsKey(handle) == false)
                {
                    LogUtils.Log("Pause audio in empty channel:" + handle, "NAudioPlayer", LogLevel.Error);
                    return false;
                }
            ...
            {
                LogUtils.Log("Pause audio failed." + ex, "NAudioPlayer", LogLevel.Error);
                return false;
            }
        }
        ...
        public bool ResumePlay(int handle)
        {
            try
            {
                if (this.channelDict.ContainsKey(handle) == false)
                {
                    LogUtils.Log("Resume play audio in empty channel:" + handle, "NAudioPlayer", LogLevel.Error);
                    return false;
                }
                this.channelDict[handle].Play();
                return true;
            ...
            {
                LogUtils.Log("Resume play audio failed." + ex, "NAudioPlayer", LogLevel.Error);
                return false;
            }
        }
        ...
        public bool SetVolume(int handle, float vol)
        {
            try
            {
                if (this.channelDict.ContainsKey(handle) == false)
                {
                    LogUtils.Log("Set volume in empty channel:" + handle, "NAudioPlayer", LogLevel.Error);
                    return false;
                }
            ...
            {
                LogUtils.Log("Set volume failed." + ex, "NAudioPlayer", LogLevel.Error);
                return false;
            }
        }
        ...
        public bool IsPlaying(int handle)
        {
            ...
            {
                LogUtils.Log("Get playing state failed." + ex, "NAudioPlayer", LogLevel.Error);
                return false;
            }
        }
        ...
        public TimeSpan GetPostion(int handle)
        {
            ...
            {
                LogUtils.Log("Get playing position failed." + ex, "NAudioPlayer", LogLevel.Error);
        ...
        public TimeSpan GetDuration(int handle)
        {
            ...
            {
                LogUtils.Log("Get playing duration failed." + ex, "NAudioPlayer", LogLevel.Error);
                return TimeSpan.FromSeconds(0);
            }
        }
        ...
        public void Dispose()
        {
            ...
            {
                LogUtils.Log("When disposing NAudioPlayer: " + ex, "NAudioPlayer", LogLevel.Warning);
            }
            finally
            {

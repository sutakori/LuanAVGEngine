...
namespace LuanEditor.LuanForms
{
    public partial class GuardForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Inst.Guard guard = new Inst.Guard();
            guard.Expr = new Expr((this.textBox1.Text.Trim()).Replace("\n", ""));
            guard.SetBlock((this.textBox2.Text.Trim()).Replace("\n", ""));
            foreach (var scene in (this.Owner as MainForm).Data[sectionname].Scenes)
            {
                if (scene.Name == scenename)
                {
                    scene.Instructions.Insert(index - sceneindex - 1, guard);
                }
            }
            string s = "        ◇条件:" + (this.textBox1.Text.Trim()).Replace("\n", ""); 

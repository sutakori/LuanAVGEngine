...
{
    public partial class SynForm : Form
    {
        /// <summary>
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            Inst.Sync sync = new Inst.Sync();
            sync.SetBlock((this.textBox2.Text.Trim()).Replace("\n", ""));
            foreach (var scene in (this.Owner as MainForm).Data[sectionname].Scenes)
            {
                if (scene.Name == scenename)
                {
                    scene.Instructions.Insert(index - sceneindex - 1, sync);
                }
            }
